...
type FooBarLister interface {
	...
	List(selector labels.Selector) (ret []*v1.FooBar, err error)
...
// List lists all FooBars in the indexer.
func (s *fooBarLister) List(selector labels.Selector) (ret []*v1.FooBar, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.FooBar))
	})
	return ret, err
}
...
type FooBarNamespaceLister interface {
	...
	Get(name string) (*v1.FooBar, error)
...
// List lists all FooBars in the indexer for a given namespace.
func (s fooBarNamespaceLister) List(selector labels.Selector) (ret []*v1.FooBar, err error) {
...
// Get retrieves the FooBar from the indexer for a given namespace and name.
func (s fooBarNamespaceLister) Get(name string) (*v1.FooBar, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("foobar"), name)
	}
	return obj.(*v1.FooBar), nil
}
